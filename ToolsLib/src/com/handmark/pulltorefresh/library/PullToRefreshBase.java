...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements
	 ...
	 * @param attrs
	 *            AttributeSet from wrapped class. Means that anything you include in the XML
	...
	/**
	 * Implemented by derived class to return whether the View is in a state where the user can
	 * Pull to Refresh by scrolling from the end.
	 * 
	...
	/**
	 * Implemented by derived class to return whether the View is in a state where the user can
	 * Pull to Refresh by scrolling from the start.
	 * 
	...
	protected final void setHeaderScroll(final int value) {
		...
		/**
		 * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them
		 * on the Header/Footer Views as they change often, which would negate any HW layer
		 * performance boost.
		 */
	...
	public static enum Mode {
		...
		/**
		 * Only allow the user to Pull from the end of the Refreshable View to refresh. The start
		 * is either the Bottom or Right, depending on the scrolling direction.
		 */
		PULL_FROM_END(0x2),
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			/**
			 * Only set mStartTime if this is the first time we're starting, else actually
